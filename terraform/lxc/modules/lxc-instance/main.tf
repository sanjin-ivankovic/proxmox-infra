...
resource "terraform_data" "write_ssh_keys" {
  ...
  # Destroy-time provisioners may only reference `self`, whose values come from
  # state — so the hostname is read from `self.output` (a plain string) and the
  # directory is left for the shell to expand. Referencing local.ssh_key_path or
  # each.key here is rejected at plan time; giving `input` a richer type breaks
  # destroys for resources whose state still holds the old string.
  provisioner "local-exec" {
    when       = destroy
    on_failure = continue
    command    = "rm -f ~/.ssh/${self.output}_id_ed25519 ~/.ssh/${self.output}_id_ed25519.pub"
  }
}
