...
#   export TF_STATE_NAME=linux-vms  # Use different names for lxc/linux-vms/windows-vms
#   terraform init \
...
terraform {
  backend "http" {
    address        = "https://gitlab.example.com/api/v4/projects/1/terraform/state/linux-vms"
    lock_address   = "https://gitlab.example.com/api/v4/projects/1/terraform/state/linux-vms/lock"
    unlock_address = "https://gitlab.example.com/api/v4/projects/1/terraform/state/linux-vms/lock"
    lock_method    = "POST"
