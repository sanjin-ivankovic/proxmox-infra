# Makefile Reference - Windows VMs

## Overview

This document provides a comprehensive reference for all Makefile commands
available in the Windows VMs Terraform project. The Makefile automates
Terraform operations for managing Proxmox Windows virtual machines, including
provisioning, configuration, state management, and integration with Ansible.

**Project**: Windows Virtual Machines on Proxmox VE
**Ansible Inventory**: `../../ansible/inventory/windows-vms-hosts.yml`
...
### Import Helpers (Windows VM-Specific)
...
| **`make import-vm-windows HOSTNAME=... VMID=... [NODE=...]`** | **Import existing Windows VM**                    |
| **`make import-guide`**                                       | **Show guide for importing existing Windows VMs** |
...
### Importing Existing Windows VMs

#### `make import-vm-windows`

Import an existing Windows VM into Terraform state.

**Usage:**

```bash
make import-vm-windows HOSTNAME=vm-name VMID=300 [NODE=pve]
```
...
```bash
make import-vm-windows HOSTNAME=dc01 VMID=300 NODE=pve
```
...
```text
module.windows_vms[0].proxmox_vm_qemu.vm["hostname"]
```
...
- Windows VMs use Cloudbase-Init for initial configuration
- Ensure Cloudbase-Init is properly configured on the VM template
- Network configuration is managed via Cloudbase-Init config drive
- See [CLOUDBASE_INIT.md](./CLOUDBASE_INIT.md) for template setup guide

#### `make import-guide`

Display a comprehensive guide for importing existing Windows VMs.
...
1. Define the VM in `instances/windows-vms.auto.tfvars`
2. Add hostname to `imported.auto.tfvars` (if preserving SSH keys)
3. Import the VM using `make import-vm-windows`
...
# 4. Define VMs in instances/windows-vms.auto.tfvars
...
# 2. Add VM definition to instances/windows-vms.auto.tfvars

# 3. Import the VM
make import-vm-windows HOSTNAME=existing-vm VMID=300
...
```bash
# 1. Modify instances/windows-vms.auto.tfvars
...
```bash
# Destroy specific VM
make destroy-target TARGET='module.windows_vms[0].proxmox_vm_qemu.vm["hostname"]'
...
### Example 1: Create and Deploy a New Windows VM
...
# 3. Add VM to instances/windows-vms.auto.tfvars
cat >> instances/windows-vms.auto.tfvars <<EOF
...
### Example 2: Import Existing Windows VM
...
make import-vm-windows HOSTNAME=existing-vm VMID=350
...
```bash
# 1. Edit instances/windows-vms.auto.tfvars to change memory from 4096 to 8192
...
3. Ensure the VM is defined in `instances/windows-vms.auto.tfvars` before
   importing
...
### Cloudbase-Init Not Working

**Issue:** VM doesn't get configured on first boot

**Solution:**

1. Verify Cloudbase-Init is installed and configured
2. Check Cloudbase-Init logs:
   `C:\Program Files\Cloudbase Solutions\Cloudbase-Init\log\cloudbase-init.log`
3. Ensure config drive is properly attached
4. Verify network configuration in Cloudbase-Init config
5. See [CLOUDBASE_INIT.md](./CLOUDBASE_INIT.md) for detailed troubleshooting
...
1. Verify template exists in Proxmox: Check storage for template
2. Ensure template name matches `windows_vm_template` in
   `terraform.tfvars.secret`
3. Template must be properly prepared with Cloudbase-Init (see
   [CLOUDBASE_INIT.md](./CLOUDBASE_INIT.md))
...
- [Terraform Windows VMs Project README](../README.md)
- [Windows VM Module](../../modules/vm-windows/README.md)
- [Ansible Inventory Module](../../modules/ansible-inventory/README.md)
- **[Cloudbase-Init Setup Guide](./CLOUDBASE_INIT.md)** -
  Comprehensive guide for creating Windows templates
...
8. **Ensure Windows template is properly prepared** with Cloudbase-Init before
   use
9. **Follow [CLOUDBASE_INIT.md](./CLOUDBASE_INIT.md)** guide for template
   creation
