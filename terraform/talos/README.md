...
3. **Terraform 1.5.0+**
4. **S3 credentials** for the Garage state backend
   (see `terraform.tfvars.secret`)
...
# Taint a specific VM
make taint RESOURCE='module.talos[0].proxmox_vm_qemu.vm["talos-cp-1"]'
...
- Secrets stored in Terraform state (encrypted at rest by Garage S3 backend)
...
```bash
# Check S3 (Garage) credentials
grep -E "access_key|secret_key" terraform.tfvars.secret

# Manually set via env (alternative to terraform.tfvars.secret)
...
- **State**: S3 backend (Garage) encrypts state at rest
- **API Tokens**: Use minimal permissions (VM management only)
- **Talos API**: Certificate-based authentication (stored in `talosconfig`)

## References

- [Terraform Makefile reference](../docs/MAKEFILE.md) — consolidated
  target table for all three Terraform projects
- [Talos Documentation](https://www.talos.dev/)
- [Talos Terraform Provider][talos-terraform]
- [Proxmox Provider Docs][proxmox-provider]
- [Terraform S3 Backend][s3-backend]
- [Garage S3-compatible store][garage]
