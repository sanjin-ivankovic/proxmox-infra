...
resource "proxmox_virtual_environment_vm" "vm" {
  ...
  lifecycle {
    ...
    # `disk` is deliberately NOT in ignore_changes, so changing disk_size in
    # talos.auto.tfvars triggers a Proxmox online disk grow (the BPG provider
    # issues qm resize). Talos detects the new size via virtio-blk and Longhorn
    # auto-rebalances replicas to use the new capacity. Trade-off: day-2 disk
    # tweaks outside Terraform (e.g. attaching extra disks via the UI) show as
    # drift.
    ignore_changes = [
      cdrom,
      efi_disk,
