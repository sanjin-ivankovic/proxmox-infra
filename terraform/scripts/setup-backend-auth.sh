...
echo -e "${CYAN}╔═══════════════════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║  Terraform GitLab Backend Authentication Setup           ║${NC}"
echo -e "${CYAN}╚═══════════════════════════════════════════════════════════╝${NC}"
echo ""
...
if [ $# -eq 1 ]; then
    ...
    echo -e "${YELLOW}📝 Enter your GitLab Personal Access Token:${NC}"
...
if [[ ! "$GITLAB_TOKEN" =~ ^gitlab-token- ]]; then
    echo -e "${RED}✗ Invalid token format. GitLab tokens should start with 'gitlab-token-'${NC}"
    exit 1
fi

# Create backend.secrets.tfvars in each terraform directory
DIRS=("lxc" "linux-vms" "windows-vms")
...
EOF
        ...
        echo -e "${YELLOW}⚠ Skipped: $dir (directory not found)${NC}"
    fi
done

echo ""
echo -e "${GREEN}✅ Backend authentication configured successfully!${NC}"
echo ""
