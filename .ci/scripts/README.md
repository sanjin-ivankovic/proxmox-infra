...
python3 .ci/scripts/detect_services.py --verbose

# Generate child pipeline
python3 .ci/scripts/generate_pipeline.py

# Build Docker image
python3 .ci/scripts/build_image.py \
...
python3 .ci/scripts/lint_yaml.py --format gitlab
...
python3 .ci/scripts/publish_github.py \
  --source . \
...
```bash
# Validate a service
./.ci/scripts/validate-service.sh pihole-1

# Pre-flight checks
./.ci/scripts/preflight-check.sh pihole-1

# Backup service
./.ci/scripts/backup-service.sh pihole-1

# Deploy a service
./.ci/scripts/deploy_service.sh pihole-1

# Health check
./.ci/scripts/health-check.sh pihole-1
```
...
```text
SC1091 (info): Not following: .ci/scripts/lib/common.sh was not specified as
...
lint-scripts:
  script:
    - shellcheck -x .ci/scripts/*.sh .ci/scripts/lib/*.sh
```
...
shellcheck -x .ci/scripts/deploy_service.sh
```
