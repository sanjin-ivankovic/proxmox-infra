...
def generate_service_jobs(service: str) -> Dict[str, Any]:
    ...
    jobs[f"validate:{service}"] = {
        ...
        "script": [f"./.ci/scripts/validate-service.sh {service}"],
    ...
    jobs[f"preflight:{service}"] = {
        ...
        "image": CI_IMAGE,
        "tags": [DEFAULT_RUNNER_TAG],
        "script": [f"./.ci/scripts/preflight-check.sh {service}"],
    ...
    jobs[f"backup:{service}"] = {
        ...
        "image": CI_IMAGE,
        "tags": [DEFAULT_RUNNER_TAG],
        "script": [f"./.ci/scripts/backup-service.sh {service}"],
    ...
    jobs[f"deploy:{service}"] = {
        ...
        "image": CI_IMAGE,
        "tags": [DEFAULT_RUNNER_TAG],
        "script": [f"./.ci/scripts/deploy_service.sh {service}"],
    ...
    jobs[f"verify:{service}"] = {
        ...
        "image": CI_IMAGE,
        "tags": [DEFAULT_RUNNER_TAG],
        "script": [f"./.ci/scripts/health-check.sh {service}"],
