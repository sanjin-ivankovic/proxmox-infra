...
### Container Restart On Daemon Start

`common_docker_restart_containers_on_start` (default `[]`) is a list of
container names to `docker restart` via a `docker.service.d` drop-in whenever
dockerd (re)starts. Use it for Docker-out-of-Docker containers that bind-mount
the daemon socket file (e.g. the GitLab runner), which otherwise stay pinned to
the pre-upgrade socket inode after a Docker upgrade. Empty writes no drop-in.
See the DooD troubleshooting entry below.
...
The role renders `/etc/docker/daemon.json` from
...
- **Log rotation** — 10MB max, 3 files (30MB per container); prevents
  loud containers from filling the disk.
- **`overlay2`** — the modern storage driver, recommended for current
  kernels.
- **Live restore** — containers keep running across daemon restarts, so
  docker engine upgrades don't bounce services.
- **BuildKit** — next-gen builder; faster, parallel, better caching.
- **Builder GC** — keeps 10GB of build cache, prunes the rest
  automatically.
...
### DooD container: "Cannot connect to the Docker daemon" after a Docker upgrade

A container that bind-mounts `/var/run/docker.sock` (Docker-out-of-Docker, e.g.
the GitLab runner) pins the socket's inode at start time. Upgrading or
restarting Docker recreates the socket at a new inode, so the long-lived
container keeps talking to the old, unlinked one. Confirm by comparing inodes:

```bash
stat -c %i /var/run/docker.sock                       # host (live)
docker exec <container> stat -c %i /var/run/docker.sock  # differs = stale
```

Fix immediately with `docker restart <container>`. To prevent recurrence, list
the container in `common_docker_restart_containers_on_start` (host_vars) so the
role writes a `docker.service.d` drop-in that restarts it after every dockerd
start.
