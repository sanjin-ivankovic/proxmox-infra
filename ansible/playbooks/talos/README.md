...
3. **03-bootstrap.yml** - Initialize etcd and Kubernetes cluster
   - Applies system:node-reader RBAC immediately after bootstrap
4. **04-configure-workload-tiers.yml** - Label worker nodes and apply
   PriorityClasses
   - All workers: `node-role.kubernetes.io/worker` (equal, no taints)
...
```bash
cd /home/user/Projects/private/proxmox-infra/ansible/talos
...
# 4. Label and taint worker nodes
...
## How these playbooks operate

Talos is an immutable, API-driven OS — there is no SSH and no package
manager on the nodes. Everything goes through `talosctl` against the
Talos API. Practically:

- **Access** — `talosctl` (mTLS with the cluster PKI in `talosconfig`),
  not SSH.
- **Config application** — `talosctl apply-config`, not `copy` /
  `template` / `lineinfile`.
- **Service management** — none from Ansible; everything is part of the
  immutable machine config.
- **Logs** — `talosctl logs <service>` via the API.
- **Reboots** — `talosctl reboot` via the API.
- **Package management** — none; you upgrade the OS by rolling a new
  Talos image with `talosctl upgrade`.

The retired K3s playbooks (which used SSH + apt + systemd) are archived under
[`.archive/ansible/k3s-on-proxmox/`](../../../.archive/ansible/k3s-on-proxmox/).
...
- **controlplane.yaml** - Applied to control planes only
  - **Layer 2 VIP** configuration (`10.10.0.59`)
  - etcd metrics
...
- **Control Plane VIP**: `10.10.0.59` (Layer 2)
  - Automatically announces from one CP at a time via gratuitous ARP
  - Endpoint: `https://10.10.0.59:6443`
...
# View logs
talosctl logs -f --nodes 10.10.0.60

# Get system info
talosctl health --nodes 10.10.0.60
talosctl version --nodes 10.10.0.60

# Read files
talosctl read /proc/meminfo --nodes 10.10.0.60

# Interactive dashboard
talosctl dashboard --nodes 10.10.0.60

# List containers
talosctl containers --nodes 10.10.0.60
```
...
- Check network connectivity: `ping 10.10.0.60`
