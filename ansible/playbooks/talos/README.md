...
4. **04-configure-workload-tiers.yml** - Label and taint worker nodes
   - worker-1: storage tier (`node-role.kubernetes.io/storage`)
   - worker-2: infra tier (`node-role.kubernetes.io/infra`)
   - worker-3: app tier (`node-role.kubernetes.io/worker` only)
