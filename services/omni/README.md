...
Authelia OIDC client `omni` (provider URL `https://authelia.example.com`). The
client is declared declaratively in argo-apps
`apps/infra/authelia/values.yaml` — Authelia clients are config-file based,
there is no admin UI to register them in. Redirect URI
`https://omni.example.com/oidc/consume`.

The client secret is stored twice: the pbkdf2 digest lives in argo-apps
kryptos (`authelia-oidc-secret`, key `omni.client.secret`) for Authelia's
side, and the plaintext half is the Komodo variable
`OMNI_OIDC_CLIENT_SECRET`. It reaches the container together with
`OMNI_OIDC_CLIENT_ID` from the host `.env`. Real access is gated by Omni
`--initial-users` (`maintainer@example.com`), matched on the email claim.
...
browse `https://omni.example.com`, log in via Authelia, and reach the Omni
dashboard.
