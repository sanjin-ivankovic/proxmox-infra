...
- **Komodo Core + FerretDB v2** — the `komodo` LXC (`10.10.0.17`),
  managed by [`ansible/roles/komodo_core`](ansible/roles/komodo_core/).
  UI at [`https://komodo.example.com`](https://komodo.example.com). The route
  carries no edge auth middleware — Komodo authenticates callers with its own
  built-in login, and GitLab's webhook reaches `/listener` directly.
